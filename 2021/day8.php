...
foreach ($src as $item) {
    $output = explode(' | ', $item);
    $detects = explode(' ', $output[0]);

    $positions = array_fill(0, 7, '');

    $digits2 = str_split(getDigitByNbCaract($detects, 2)[0]);
    $digits3 = str_split(getDigitByNbCaract($detects, 3)[0]);
    $digits4 = str_split(getDigitByNbCaract($detects, 4)[0]);
    $digits7 = str_split(getDigitByNbCaract($detects, 7)[0]);
    
    $positions[0] = array_values(array_diff($digits3, $digits2))[0]; // Trouver position 0
    
    // Trouver positions 1 et 4
    $digits5 = getDigitByNbCaract($detects, 5);
    $nb3 = 0;

    foreach ($digits5 as $key => $d5) {
        $d_array = str_split($d5);

        if (count(array_intersect($d_array, $digits2)) == 2) {
            $nb3 = $key;
            break;
        }
    }
    ...
    foreach ($digits5 as $key => $d5) {
        ...
        if ($key != $nb3 && count(array_intersect($d_array, $digits4)) == 2) { // NB 2
            $positions[2] = array_values(array_intersect($d_array, $digits2))[0];
            $positions[4] = array_values(array_diff($d_array, str_split($digits5[$nb3])))[0];
        }

        if ($key != $nb3 && count(array_intersect($d_array, $digits4)) == 3) { // NB 5
            $positions[5] = array_values(array_intersect($d_array, $digits2))[0];
            $positions[1] = array_values(array_diff($d_array, str_split($digits5[$nb3])))[0];
        }
    }

    // Trouver position 6
    $digits6 = getDigitByNbCaract($detects, 6);

    foreach ($digits6 as $key => $d6) {
        $d_array = str_split($d6);

        if (count(array_intersect($d_array, $positions)) == 5) {
            $positions[6] = array_values(array_diff($d_array, $positions))[0];
            break;
        }
    }

    $digits7 = str_split(getDigitByNbCaract($detects, 7)[0]);
    $positions[3] = array_values(array_diff($digits7, $positions))[0];

    /////////////////////////////////

    $digits = explode(' ', $output[1]);
    $number = '';
    ...
    foreach ($digits as $d) {
        ...
        foreach ($d_array as $letter) {
            $pattern[] = array_search($letter, $positions);
        }

        $number .= getValueByLines($pattern);
    }
...
echo 'Final number: ' . $total . "\n";

function getDigitByNbCaract($array, $digit) {
    $returns = [];
    
    foreach ($array as $item) {
        if (strlen($item) == $digit) {
            $returns[] = $item;
        }
    }
...
function getValueByLines($array) {
    ...
    foreach ($patterns as $key => $pattern) {
        if (count($pattern) == count($array) && count(array_intersect($pattern, $array)) == count($array)) {
            $returnValue = $key;
        }
    }
